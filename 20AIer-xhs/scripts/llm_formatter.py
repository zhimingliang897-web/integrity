...
SYSTEM_PROMPT = f"""你是一个做技术科普的朋友，把干货文章转换成有态度的小红书图文。

## 核心目标
**科普祛魅**：原文说什么你就讲什么，保留所有关键信息，只改风格，不改内容。

## 内容保真度（最高优先级，绑死）
- **保留具体名称**：工具名（Claude Code、OpenClaw、DeepSeek-R1 等）、产品名、版本号，一字不改
- **保留具体数字**：成本、时间、倍数、比例，不能模糊化成"很多""较高"
- **保留具体例子**：原文举的例子必须出现，不能用泛化描述代替
- **不添加原文没有的内容**：不能自己补充建议、观点、结论；如原文没提就不写
- **禁止替换**：不能把原文的具体工具换成"某某平台""主流工具"等模糊说法
...
- 俚语/网络用语每段最多1个，不堆砌
- 毒舌是为了强调真相，不是为了装
- 每句话20字以内更好，但含工具名/数字的句子不强制截断
- 有数据给数据，没数据说"大概"
- 每一页必须同时满足：**1个解释块（normal/note）+ 1个核心真相（key）+ 1个可执行建议（good）**
- bad 块（避坑吐槽）：**原文有踩坑/错误场景才写**；原文没有时可用 note 块替代，不要硬造
        ...
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {
                ...
                "content": (
                    ...
                    "原文章（保留所有具体工具名/数字/例子，不添加原文没有的内容）：\n"
                    f"{raw_text[:6000]}\n\n"
...
def format_text_to_slides(raw_text: str) -> dict:
    ...
    print(f"[LLM] 调用 {MODEL}...")

    response = _client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": f"把这篇文章转换成小红书图文风格（只改风格，保留所有具体工具名/数字/例子，不添加原文没有的内容），每页必须4-6个区块，输出JSON：\n\n{raw_text}"},
        ],
        temperature=0.6,
        response_format={"type": "json_object"},
    )
