"""
gen_text_pre.py — 调用通义千问 API 自动生成对话文本 + 通义万相生成标准四宫格插图（连续四分镜故事）
用法: python gen_text_pre.py <项目文件夹名> <场景描述>
例如: python gen_text_pre.py 6买咖啡 "在咖啡店点单，一个男生第一次去星巴克不知道怎么点"
...
MODEL = os.environ.get("DASHSCOPE_TEXT_MODEL", "qwen-plus-2025-12-01")           # 文本模型
IMAGE_MODEL = os.environ.get("DASHSCOPE_IMAGE_MODEL", "qwen-image-max")  # 图片模型
...
# wanx / old qwen-image (image-generation)
...
def generate_image(scene_description, output_path, panels=None):
    """
    通过 DashScope 生成插图（智能路由：支持原生异步 polling 和 OpenAI 兼容接口）。
    ...
    if IMAGE_MODEL in NATIVE_MODELS:
        ...
        if IMAGE_MODEL == "qwen-image-max":
            ...
            payload = {
                ...
                "parameters": {
                    "size": "1024*1024",  # qwen-image-max 支持此分辨率
                    "n": 1,
                    "prompt_extend": True,
                    "watermark": False
                }
        ...
        # 处理同步结果（qwen-image-max 可能直接返回图片）
        ...
        for _ in range(60):  # 最多等待 120 秒
            ...
            if task_status == "SUCCEEDED":
                ...
                # 尝试解析 qwen-image-max 结构 (output -> choices -> message -> content -> image)
                ...
                # 尝试解析通用结构 (output -> results -> url 或 output -> url)
                if not img_url:
...
def print_image_guide_pre(panels, scene_description, input_dir, project_name):
    ...
    lines += [
        ...
        "=" * 62,
        "图片准备好后，运行：",
        f"  python make_video_pre.py {project_name}",
        sep,
    ]
    content = "\n".join(lines)
    print(content)
...
def main():
    ...
    if len(args) < 2:
        print("用法: python gen_text_pre.py <项目文件夹名> <场景描述> [--no-image] [--overwrite]")
    ...
    print("  素材生成完毕！")
    print("=" * 50)
    print(f"\n  下一步: python make_video_pre.py {project_name}")
    if not no_image:
