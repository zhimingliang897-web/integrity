...
## v10 — 四宫格模式支持 Qwen-Image-Max
- **API 升级**：`gen_text_pre.py` 现已支持 `qwen-image-max` 等需要异步轮询（Task API）的模型。
- **智能路由**：自动检测模型名称，智能选择 "OpenAI兼容模式"（wanx）或 "原生轮询模式"（qwen-image系列）。
- **稳定性**：增加了 API 限流保护和超时重试机制。
