...
def _configure_stdio_encoding() -> None:
    """避免 Windows 默认编码导致打印标题时崩溃。"""
    for stream in (sys.stdout, sys.stderr):
        try:
            stream.reconfigure(encoding="utf-8", errors="replace")
        except Exception:
            # 某些环境/对象不支持 reconfigure，忽略即可
            pass
...
def main():
    ...
    # ---------- topic 子命令 ----------
    sp_topic = subparsers.add_parser("topic", help="话题搜索+批量抓取+分析 (一站式)")
    sp_topic.add_argument(
        "--keyword", "-k",
        required=True,
        help="话题关键词",
    )
    sp_topic.add_argument(
        ...
        help='搜索平台 (逗号分隔，默认全部: bilibili,douyin,xiaohongshu)',
    )
    sp_topic.add_argument(
        "--max-search", "-ms",
        type=int,
        default=5,
        help="每个平台的最大搜索结果数 (默认: 5)",
    )
    sp_topic.add_argument(
        "--max-comments", "-mc",
        type=int,
        default=50,
        help="每个视频/帖子的最大评论数 (默认: 50)",
    )
    sp_topic.add_argument(
        "--speed", "-s",
        choices=["fast", "normal", "slow", "safe"],
        default=None,
        help="速度档位 (默认从 config.py 读取)",
    )
    sp_topic.add_argument(
        "--output", "-o",
        default=None,
...
def cmd_topic(args):
    """执行话题分析流水线"""
    from topic.pipeline import run_topic_pipeline
