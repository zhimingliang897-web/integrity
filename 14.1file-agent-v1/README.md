...
| 文件名 | 说明（对他人） | 样式/格式 |
|--------|----------------|-----------|
| **config.json** | Web 登录密码、Session 签名密钥、LLM API Key（阿里百炼 DashScope）、Natapp 内网穿透 Token、邮箱发件人及 SMTP 授权码、存储路径与挂载目录 | JSON 格式，字段见上方示例或 `config.example.json`；`llm.api_key` 格式为 `sk-...`，`email.password` 为 QQ 邮箱 16 位 SMTP 授权码，`natapp.token` 为 Natapp 隧道 Token |

**自己使用**：从本仓库的 **`_secrets/14.1file-agent-v1/config.json`** 拷贝到本目录（文件名不变）即可。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/files/star | 收藏文件 |
| DELETE | /api/files/star | 取消收藏 |
| GET | /api/files/starred | 获取收藏列表 |

### 邮箱发送

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/files/email | 发送文件到邮箱 |
...
1. 点击文件列表中的 **☆** 按钮收藏文件
2. 点击侧边栏 **收藏** 查看所有收藏的文件
3. 再次点击 **⭐** 可取消收藏

### 邮箱发送

1. 在设置页面配置发件邮箱（QQ邮箱需使用授权码）
2. 选择要发送的文件
3. 点击工具栏 **📧 发送邮件** 按钮
4. 输入收件人邮箱，点击发送

> 注意：QQ邮箱需要开启SMTP服务并获取16位授权码作为密码

### AI助手发送邮件

除了手动发送，你也可以通过AI助手发送邮件：
- 对AI说 "发邮件给 xxx@qq.com，附件是 xxx文件"
- AI会自动完成文件附件和发送
