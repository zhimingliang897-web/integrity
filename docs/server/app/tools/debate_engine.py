...
class DebateEngine:
    ...
    def __init__(self, topic: str, pro_position: str, con_position: str,
        ...
        # 运行时配置（必须由调用方提供）
        if debaters is None or providers is None:
            raise ValueError("debaters 和 providers 参数必须提供")
        self._debaters = debaters
        self._providers = providers
        self._judge_config = params.get("judge_config") if params else None

        params = params or {}
        self._max_words = int(params.get("max_words", 200))
        self._free_debate_rounds = int(params.get("free_debate_rounds", 6))
        self._debate_time_limit = float(params.get("debate_time_limit", 300))
    ...
    def init_clients(self):
        ...
        # 裁判客户端（使用配置中指定的 provider 或第一个可用的）
        if self._judge_config and "provider" in self._judge_config:
            judge_provider_key = self._judge_config["provider"]
        ...
        if not judge_provider:
            raise ValueError("无法找到裁判的 LLM 服务商配置")
...
（宣布胜方及理由，1-2句）"""
    ...
    def run_debate(self):
        ...
        judge_voice = self._judge_config.get("voice", "zh-CN-YunyangNeural") if self._judge_config else "zh-CN-YunyangNeural"
        self.history.append({
            "id": "judge",
            "name": "AI裁判",
            "side": "neutral",
        ...
        yield {"event": "judge", "data": {"content": judge_result, "voice": judge_voice}}
